...
AacSource::AacSource(uint32_t samplerate=48000, uint32_t channels=2, bool hasAdts=true) : samplerate_(samplerate)
, channels_(channels), hasAdts_(hasAdts)
{
    payload_ = 97;
    mediaType_ = AAC;
    clockRate_ = samplerate;
}

AacSource *AacSource::Create(uint32_t samplerate, uint32_t channels, bool hasAdts)
{
...
bool AacSource::HandleFrame(MediaFrame frame)
{
	...
	rtp_pkt.size = frame_size + RTP_HEADER_SIZE + AU_SIZE;
	rtp_pkt.last = 1;

	rtp_pkt.data.get()[RTP_HEADER_SIZE + 0] = AU[0];
	rtp_pkt.data.get()[RTP_HEADER_SIZE + 1] = AU[1];
	rtp_pkt.data.get()[RTP_HEADER_SIZE + 2] = AU[2];
	rtp_pkt.data.get()[RTP_HEADER_SIZE + 3] = AU[3];

	memcpy(rtp_pkt.data.get() + RTP_HEADER_SIZE + AU_SIZE, frame_buf, frame_size);

	if (sendFrameCallback_) {
		sendFrameCallback_(rtp_pkt);
	}
